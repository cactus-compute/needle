...
**Data synthesis.** `needle generate-data` writes training examples from your tool schemas, through OpenRouter by default or through [OrcaRouter](https://www.orcarouter.ai), an OpenAI-compatible gateway for both models and agents, when you pass `--provider orcarouter`:
...
Rather than pasting a key you can authorize with your own account: `needle connect` runs OAuth 2.0 + PKCE and stores the resulting `sk-orca-…` key, which is billed to you and revocable from your console at any time (`--oob` pastes a code instead when no callback is reachable). `needle models --provider orcarouter` lists what that key can reach, straight from `GET /v1/models` and filtered by capability, and the playground's model dropdown is that same live list rather than a hand-written sample. `ORCA_AUTH_BASE_URL` and `ORCA_API_BASE_URL` override the auth and inference origins separately; `ORCA_BASE_URL` supplies both for a self-hosted deployment. Prompts can carry one image with `--input-modality image --image-url <https URL or data: URI>`, which narrows the model list to catalog entries that declare an `image` input and refuses any other model before a request is sent.
