...
11. **Saliency is text-only.** Speech warmup gradients are currently discarded — saliency ranks FFN neurons based on text steps only. This is acceptable since text is the primary task, but for speech-heavy deployments the saliency ranking may under-represent speech-relevant neurons. A future experiment could accumulate speech gradients into the same saliency buffer and compare sub-model speech PPL.
